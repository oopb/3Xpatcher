...
func renderInbound(r InboundRecord) ([]any, []string, error) {
	...
	case ProtocolTUIC:
		...
		if s.TLS.Reality != nil && s.TLS.Reality.Enabled {
			return nil, nil, errors.New("Reality is not supported by TUIC/QUIC")
		}
		m := baseInbound("tuic", r)
		applyListen(m, s.ListenSettings)
		applyQUIC(m, s.QUICSettings)
		m["users"] = s.Users
	...
	case ProtocolNaive:
		...
		if s.TLS.Reality != nil && s.TLS.Reality.Enabled {
			return nil, nil, errors.New("Reality is not supported by sing-box Naive outbound")
		}
	...
	case ProtocolShadowTLS:
		...
		if strings.TrimSpace(s.HandshakeServer) == "" && s.WildcardSNI != "all" {
		...
		applyListen(outer, s.ListenSettings)
		outer["version"] = 3
		outer["users"] = s.Users
		...
		if strings.TrimSpace(s.HandshakeForServerNameJSON) != "" {
			var routes map[string]map[string]any
			if err := json.Unmarshal([]byte(s.HandshakeForServerNameJSON), &routes); err != nil {
				return nil, nil, fmt.Errorf("invalid handshakeForServerNameJson: %w", err)
			}
			if len(routes) > 0 {
				outer["handshake_for_server_name"] = routes
			}
		}
		if s.StrictMode {
		...
		inner := map[string]any{"type": "shadowsocks", "tag": innerTag, "listen": "127.0.0.1", "network": "tcp", "method": method, "password": s.InnerPassword}
...
func baseInbound(kind string, r InboundRecord) map[string]any {
	return map[string]any{"type": kind, "tag": r.Tag, "listen": r.Listen, "listen_port": r.Port}
}

func applyListen(m map[string]any, s ListenSettings) {
	if s.BindInterface != "" {
		m["bind_interface"] = s.BindInterface
	}
	if s.RoutingMark != 0 {
		m["routing_mark"] = s.RoutingMark
	}
	if s.ReuseAddr {
		m["reuse_addr"] = true
	}
	...
	if s.DisableTCPKeepAlive {
		m["disable_tcp_keep_alive"] = true
	}
	if s.TCPKeepAlive != "" {
		m["tcp_keep_alive"] = s.TCPKeepAlive
	}
	if s.TCPKeepAliveInterval != "" {
		m["tcp_keep_alive_interval"] = s.TCPKeepAliveInterval
	}
	if s.UDPFragment != nil {
		m["udp_fragment"] = *s.UDPFragment
	}
	if s.UDPTimeout != "" {
		m["udp_timeout"] = s.UDPTimeout
	}
}
...
func applyQUIC(m map[string]any, s QUICSettings) {
	...
	if s.KeepAlivePeriod != "" {
		m["keep_alive_period"] = s.KeepAlivePeriod
	}
	if s.StreamReceiveWindow != nil {
		m["stream_receive_window"] = s.StreamReceiveWindow
	}
	if s.ConnectionReceiveWindow != nil {
		m["connection_receive_window"] = s.ConnectionReceiveWindow
	}
	if s.MaxConcurrentStreams > 0 {
		m["max_concurrent_streams"] = s.MaxConcurrentStreams
	}
	if s.InitialPacketSize > 0 {
		m["initial_packet_size"] = s.InitialPacketSize
	}
	if s.DisablePathMTUDiscovery {
...
func renderTLS(s TLSSettings) (map[string]any, error) {
	...
	realityEnabled := s.Reality != nil && s.Reality.Enabled
	if realityEnabled {
		if strings.TrimSpace(s.ServerName) == "" {
			return nil, errors.New("Reality serverName/SNI is required")
		}
		if strings.TrimSpace(s.Reality.HandshakeServer) == "" {
			return nil, errors.New("Reality handshake server is required")
		}
		port := s.Reality.HandshakePort
		if port == 0 {
			port = 443
		}
		if port < 1 || port > 65535 {
			return nil, errors.New("invalid Reality handshake port")
		}
		privateKey, err := base64.RawURLEncoding.DecodeString(strings.TrimSpace(s.Reality.PrivateKey))
		if err != nil || len(privateKey) != 32 {
			return nil, errors.New("Reality private key must be a 32-byte raw URL-safe base64 X25519 key")
		}
		if len(s.CurvePreferences) > 0 {
			return nil, errors.New("curve preferences are unavailable in Reality")
		}
		m["reality"] = map[string]any{
			"enabled":     true,
			"handshake":   map[string]any{"server": s.Reality.HandshakeServer, "server_port": port},
			"private_key": s.Reality.PrivateKey,
		}
		if len(s.Reality.ShortIDs) > 0 {
			m["reality"].(map[string]any)["short_id"] = s.Reality.ShortIDs
		}
		if s.Reality.MaxTimeDifference != "" {
			m["reality"].(map[string]any)["max_time_difference"] = s.Reality.MaxTimeDifference
		}
	} else if strings.TrimSpace(s.CertificatePath) != "" || strings.TrimSpace(s.KeyPath) != "" {
	...
	if s.MinVersion != "" {
		m["min_version"] = s.MinVersion
	}
	if s.MaxVersion != "" {
		m["max_version"] = s.MaxVersion
	}
	if len(s.CipherSuites) > 0 {
		m["cipher_suites"] = s.CipherSuites
	}
	if len(s.CurvePreferences) > 0 {
		m["curve_preferences"] = s.CurvePreferences
	}
	return m, nil
}
