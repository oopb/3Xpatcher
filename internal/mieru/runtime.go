...
func DefaultRuntime() Runtime {
	return Runtime{BinaryPath: DefaultBinaryPath, ConfigDir: DefaultConfigDir, ServiceFmt: DefaultServiceFmt}
}
...
func (r Runtime) CheckBytes(config []byte) error {
	...
	cmd.Env = append(os.Environ(),
		"MITA_CONFIG_FILE="+filepath.Join(dir, "server.conf.pb"),
		"MITA_UDS_PATH="+filepath.Join(dir, "mita.sock"),
	)
...
func (r Runtime) Stop(id int) error {
	...
	if err != nil {
		...
		if strings.Contains(text, "not loaded") || strings.Contains(text, "not found") { return nil }
...
func (r Runtime) Remove(id int) error {
	if err := r.Stop(id); err != nil { return err }
...
func (r Runtime) Status(id int) (string, error) {
	out, err := exec.Command("systemctl", "is-active", r.service(id)).CombinedOutput()
	status := strings.TrimSpace(string(out))
	if status != "" { return status, nil }
	return status, err
}
...
func (r Runtime) ConfiguredIDs() ([]int, error) {
	...
	for _, entry := range entries {
		...
		id, err := strconv.Atoi(strings.TrimSuffix(entry.Name(), ".json"))
		if err == nil && id > 0 { ids = append(ids, id) }
	}
	return ids, nil
}
...
func (r Runtime) Version() (string, error) {
	...
	if err != nil { return "", fmt.Errorf("mita version: %w: %s", err, strings.TrimSpace(string(out))) }
