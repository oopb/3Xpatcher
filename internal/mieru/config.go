...
func BuildServerConfig(r Record) ([]byte, error) {
	...
	if r.Port < 1025 || r.Port > 65535 {
		return nil, fmt.Errorf("Mieru port %d is outside 1025..65535", r.Port)
	}
	if len(r.Users) == 0 {
	...
	if transport != "TCP" && transport != "UDP" {
		return nil, fmt.Errorf("invalid Mieru transport %q", r.Settings.Transport)
	}

	binding := map[string]any{"protocol": transport}
	if r.Settings.PortRangeEnd > 0 {
		if r.Settings.PortRangeEnd < r.Port || r.Settings.PortRangeEnd > 65535 {
			return nil, fmt.Errorf("Mieru port range %d-%d is invalid", r.Port, r.Settings.PortRangeEnd)
		}
	...
	for _, u := range r.Users {
		m := map[string]any{"name": u.Name, "password": u.Password}
