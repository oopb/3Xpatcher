...
func TestMieruMetricDelta(t *testing.T) {
	...
	if d := mieruMetricDelta(7, base); len(d) != 0 { t.Fatalf("baseline billed: %#v", d) }
...
func TestParseMetricsJSONWithLogPrefix(t *testing.T) {
	...
	if m["traffic"]["UploadBytes"] != 12 || m["traffic"]["DownloadBytes"] != 34 { t.Fatalf("bad parse: %#v", m) }
}
...
func TestParseRecentUsers(t *testing.T) {
	...
	if len(users) != 1 || users[0] != "alice@example.com" { t.Fatalf("unexpected recent users: %#v", users) }
}
