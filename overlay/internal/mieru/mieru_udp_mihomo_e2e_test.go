...
func TestMieruUDPE2E(t *testing.T) {
	...
	const username = "mieru-udp-e2e@example.com"
	const password = "3xpatcher-mieru-udp-e2e-secret"
	serverPort := freeUDPPort(t)
	mixedPort := freeTCPPort(t)

	target := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/plain")
		_, _ = w.Write([]byte("3xpatcher-mieru-udp-e2e-ok"))
	}))
	defer target.Close()

	cfg, err := BuildServerConfig(Record{
		ID:     992,
		Remark: "mieru-udp-e2e",
		Port:   serverPort,
		Settings: Settings{
			Transport:       "UDP",
			MTU:             1400,
			...
			DNSDualStack:    "PREFER_IPv4",
		},
		Users: []User{{Name: username, Password: password}},
	})
	if err != nil {
		t.Fatalf("BuildServerConfig UDP: %v", err)
	}
	if !bytes.Contains(cfg, []byte(`"protocol": "UDP"`)) && !bytes.Contains(cfg, []byte(`"protocol":"UDP"`)) {
		t.Fatalf("rendered Mieru config lost UDP transport: %s", cfg)
	}

	dir, err := os.MkdirTemp("/tmp", "3xpatcher-mieru-udp-e2e-")
	if err != nil {
		t.Fatal(err)
	}
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatalf("UDP mita never became RUNNING: %v\nstatus=%s\nlog=%s", statusErr, out, mitaLog.String())
		}
...
`, mixedPort, serverPort, username, password)
	...
	if err := mihomoCmd.Start(); err != nil {
		t.Fatalf("start Mihomo UDP: %v", err)
	}
	...
	for i := 0; i < 8; i++ {
		...
		if curlErr == nil && strings.Contains(string(out), "3xpatcher-mieru-udp-e2e-ok") {
			return
		}
		if i == 7 {
			t.Fatalf("real Mieru UDP request through Mihomo failed: %v\nresponse=%s\nmita=%s\nmihomo=%s", curlErr, string(out), mitaLog.String(), mihomoLog.String())
		}
