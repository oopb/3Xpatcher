...
func TestManualGenerateReturnsMetadataAndReuses(t *testing.T) {
	...
	if !first.Created || first.CertificatePath == "" || first.KeyPath == "" || first.NotAfter == "" { t.Fatalf("bad first result: %#v", first) }
	second, err := GenerateSelfSignedCertificate("www.apple.com", 365)
	if err != nil { t.Fatal(err) }
	if second.Created { t.Fatalf("expected reuse: %#v", second) }
	if first.CertificatePath != second.CertificatePath || first.KeyPath != second.KeyPath { t.Fatalf("paths changed: %#v %#v", first, second) }
}
...
func TestLegacy06SelfSignedFieldsStillWork(t *testing.T) {
	...
	if err := installTLSFromStream(settings, stream); err != nil { t.Fatal(err) }
	if settings["tls"].(map[string]any)["serverName"] != "www.cloudflare.com" { t.Fatal("legacy SNI migration failed") }
}
