...
// genSingboxLink adds the supplemental protocols to the panel's existing raw
// subscription. The identity comes from ClientRecord through clientForLink;
// address/remark/Host handling stays in the normal SubService path.
...
func (s *SubService) buildSingboxEndpointLink(inbound *model.Inbound, client model.Client, settings, stream, ep map[string]any, host string, port int) string {
	...
	applySingboxTLSLinkParams(stream, ep, params)
	...
	case model.AnyTLS:
		...
		base := fmt.Sprintf("anytls://%s@%s", encodeUserinfo(client.Password), joinHostPort(host, port))
		return buildLinkWithParams(base, params, remark)
	case model.ShadowTLS:
		...
		base := fmt.Sprintf("shadowtls://%s@%s", encodeUserinfo(client.Password), joinHostPort(host, port))
		return buildLinkWithParams(base, params, remark)
	case model.Naive:
		if client.Password == "" {
			return ""
		}
		base := fmt.Sprintf("naive+https://%s:%s@%s", encodeUserinfo(client.Email), encodeUserinfo(client.Password), joinHostPort(host, port))
		return buildLinkWithParams(base, params, remark)
	default:
		return ""
	}
...
func applySingboxTLSLinkParams(stream map[string]any, ep map[string]any, params map[string]string) {
