...
subprocess.run(
    [sys.executable, str(Path(__file__).resolve().parent / 'v25-tuic-export-bridge.py'), sys.argv[1]],
    check=True,
)
subprocess.run(
...
subprocess.run(
    [sys.executable, str(Path(__file__).resolve().parent / 'v25-tuic-export-restore.py'), sys.argv[1]],
    check=True,
)
