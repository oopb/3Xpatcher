...
# V11.8 TUIC: real Clash Verge A/B testing isolated the regression to one
# missing field: skip-cert-verify:true. Keep the pre-Mieru TUIC shape otherwise:
# udp:true stays, no udp-relay-mode is invented, and SNI/ALPN are copied exactly.
...
! grep -q 'proxy\["udp-relay-mode"\]' overlay/internal/sub/singbox_clash.go
...
grep -q 'TestTUICClashRestoresPreMieruShapeWithRequiredSkipCertVerify' overlay/internal/sub/singbox_links_test.go
grep -q 'TestTUICClashDoesNotInventALPNOrUDPRelayMode' overlay/internal/sub/singbox_links_test.go
grep -q 'TestTUICMihomoE2E' overlay/internal/sub/tuic_mihomo_e2e_test.go
! test -e overlay/internal/sub/tuic_selfsigned_mihomo_e2e_test.go
...
for f in \
  ...
  overlay/internal/sub/tuic_mihomo_e2e_test.go \
